...
namespace CompleteProject
{
	...
	public class BhanusPurchaser : MonoBehaviour , IStoreListener
	{
		...
		private static string k1MonkeynutConsumable =    "1monkeynutconsumable"; 
		private static string k2MonkeynutsConsumable =    "2monkeynutsconsumable"; 
		...
		private static string kOneMonkeynutGooglePlayConsumable =    "test01";        // Google Play Store identifier for the consumable product.
		private static string kTwoMonkeynutsGooglePlayConsumable =    "test02";        // Google Play Store identifier for the consumable product.
		...
		public void InitializePurchasing() 
		{
			...
			builder.AddProduct(kProductIDConsumable, ProductType.Consumable, new IDs(){{ kProductNameAppleConsumable,       AppleAppStore.Name },{ kProductIDConsumable , GooglePlay.Name },});// Continue adding the non-consumable product.
			builder.AddProduct(k1MonkeynutConsumable , ProductType.Consumable , new IDs() {{kOneMonkeynutGooglePlayConsumable , GooglePlay.Name}});
			builder.AddProduct(k2MonkeynutsConsumable , ProductType.Consumable , new IDs() {{kTwoMonkeynutsGooglePlayConsumable , GooglePlay.Name}});
		...
		public void BuyOneMonkeynut()
		{
			...
			BuyProductID(k1MonkeynutConsumable);
		}

		public void BuyTwoMonkeynuts()
		{
			// Buy the consumable product using its general identifier. Expect a response either through ProcessPurchase or OnPurchaseFailed asynchronously.
			BuyProductID(k2MonkeynutsConsumable);
		}
		...
		public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args) 
		{
			...
			if(String.Equals(args.purchasedProduct.definition.id , k1MonkeynutConsumable , StringComparison.Ordinal))
			{
			...
			else if(String.Equals(args.purchasedProduct.definition.id , k2MonkeynutsConsumable , StringComparison.Ordinal))
			{
				Debug.Log(string.Format("ProcessPurchase: PASS. Product: '{0}'" , args.purchasedProduct.definition.id));//If the consumable item has been successfully purchased, add 100 coins to the player's in-game score.
				scoreManagementScript.superChimpScoreValue += 2;
			}
