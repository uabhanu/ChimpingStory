...
{
    bool _bMiscObjAdded;
    const float _difficultyHeightLevel = 1.25f , _difficultyTileWidth = 1.25f , _tileWidth = 1.25f;
    ...
    [SerializeField] float _difficultyIncreaseRate , _minMiddleCounterValue;
	...
	void Start() 
	{
        ...
        m_gameSpeed = 5f;
        Invoke("GameDifficultyIncrease" , _difficultyIncreaseRate);
	...
	void SpawnTile()
    {
		if(_blankCounter > 0 && GameManager.m_gameDifficulty > 0)
        {
			SetTile("PF_Blank");
			_blankCounter--;
            return;
        ...
        else if(_lastTile == "PF_GroundMiddle" && GameManager.m_gameDifficulty >= 1)
        {
			SetTile("PF_GroundRight");
		}
	}
